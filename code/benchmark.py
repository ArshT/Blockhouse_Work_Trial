...
class Benchmark:
    ...
    def calculate_vwap(self, idx, shares):
        ...
        for i, size in enumerate(bid_sizes):
            ...
            if cumsum >= shares:
                break
        return np.sum(np.array(bid_prices[0][:idx+1]) * np.array(bid_sizes[0][:idx+1])) / np.sum(np.array(bid_sizes[0][:idx+1]))
...
if __name__ == '__main__':
    data_df = pd.read_csv('merged_bid_ask_ohlcv_data.csv')
    benchmark = Benchmark(data_df)

    # Generate TWAP trades
    twap_trades = benchmark.get_twap_trades(data_df, 1000)
    print("TWAP Trades:", twap_trades)

    # # Generate VWAP trades
    # vwap_trades = benchmark.get_vwap_trades(data_df, 1000)
    # print("VWAP Trades:", vwap_trades)